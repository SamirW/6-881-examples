...
class SAC(object):
    def __init__(self, num_inputs, action_space, max_action, args):
        ...
        if self.policy_type == "Gaussian":
            self.policy = GaussianPolicy(self.num_inputs, self.action_space, args.hidden_size, max_action)
    ...
    def update_parameters(self, state_batch, action_batch, reward_batch, next_state_batch, mask_batch, updates):
        ...
        if self.policy_type == "Gaussian":
            ...
            next_q_value = reward_batch + mask_batch * self.gamma * target_value  # Reward Scale * r(st,at) - gamma*V(target)(st+1))
        else:
            """
            ...
            next_q_value = reward_batch + mask_batch * self.gamma * target_critic  # Reward Scale * r(st,at) - gamma*Q(target)(st+1)
        
        
        """
        Soft Q-function parameters can be trained to minimize the soft Bellman residual
        JQ = (Est,at)~D[0.5(Q1(st,at) - r(st,at) - gamma*(Est+1~p[V(st+1)]))^2]
        delta_JQ = delta_Q(st,at)(Q(st,at) - r(st,at) - gamma*V(target)(st+1))
        """
        ...
        if self.policy_type == "Gaussian":
            ...
            JV = Est~D[0.5(V(st) - (Eat~pi[Qmin(st,at) - log pi(at|st)]))^2]
            delta_JV = delta_V(st)(V(st) - Q(st,at) + log_pi(at|st))
            """
